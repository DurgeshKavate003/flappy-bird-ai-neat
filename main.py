...
WIN_WIDTH = 500
WIN_HEIGHT = 700
WIN_TITLE = 'Flappy Bird with NEAT AI'
...
STAT_FONT = pygame.font.SysFont("Copperplate Gothic", 25, True)
...
def draw_window(win, birds, pipes, base, score, gen):
    ...
    win.blit(text, (WIN_WIDTH - 120 - text.get_height(), 10))
        ...
        for pipe in pipes:
            for x, bird in enumerate(birds):
                if pipe.collide(bird):
                    # ge[x].fitness -= 1 # birds.remove(bird)
                    birds.pop(x)
                    nets.pop(x)
                    ge.pop(x)
